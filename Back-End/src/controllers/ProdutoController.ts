...
export const findAllPizzas = async (req: Request, res: Response, next: NextFunction) => {
    const listaPizza = await _produtoServicos.Consulte();

    listaPizza ? res.json(listaPizza) : next(new Error("Null list"));
}

export const FindByIDPizza = async (req: Request, res: Response, next: NextFunction) => {
